...
// 不能识别selector时调用
- (void)doesNotRecognizeSelector:(SEL)sel {
    // 显示错误，结果例如：  -[NewTarget haha1]: unrecognized selector sent to instance 0x7ff5ba500430
    _objc_fatal("-[%s %s]: unrecognized selector sent to instance %p",
...
//实例执行 @selector ，带参数，同 + (id)performSelector:(SEL)sel
...
//实例执行 @selector
- (id)performSelector:(SEL)sel {
    //如果sel为空，就调用 doesNotRecognizeSelector: ，不能识别selector
    if (!sel) [self doesNotRecognizeSelector:sel];
